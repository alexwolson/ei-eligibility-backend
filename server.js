...
const db = new sqlite3.Database('./ei_eligibility.db');
...
const isDataStale = (retrievalDate) => {
    return (Date.now() - retrievalDate) > THIRTY_DAYS_IN_MS;
};
...
app.get('/scrape', async (req, res) => {
    ...
    db.get(`SELECT * FROM postal_code_data WHERE postal_code = ?`, [postalCode], async (err, postalRow) => {
        if (err) {
        ...
        try {
            ...
            const postalData = {
                ...
                date_retrieved: Date.now()
            };

            // Insert or update postal code data in the database
            db.run(
                `INSERT OR REPLACE INTO postal_code_data (postal_code, census_subdivision_name, common_name, census_division_name, ei_economic_region_name, ei_economic_region_url, date_retrieved)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
const getEconomicRegionData = (regionName, regionURL) => {
    return new Promise((resolve, reject) => {
        ...
        db.get(`SELECT * FROM economic_region_data WHERE economic_region_name = ?`, [regionName], async (err, econRow) => {
            if (err) {
                console.error('Database error:', err);
                return reject(err);
            }
            ...
            try {
                ...
                econRegionData.date_retrieved = Date.now();

                // Insert or update economic region data in the database
                db.run(
                    `INSERT OR REPLACE INTO economic_region_data (economic_region_name, province, economic_region_code, unemployment_rate, insured_hours_required, min_weeks_payable, max_weeks_payable, best_weeks_required, date_retrieved)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
